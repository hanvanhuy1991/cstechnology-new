...
@section('page-header')
    <x-page-header fixed="true">
        <x-slot name='title'>
            <h4><i class="icon-folder icon-2x mr-2"></i> <span class="font-weight-semibold">{{ __('Taxonomies') }}</span></h4>
        </x-slot>
        <x-slot name='right'>
            <div class="header-elements d-none">
                <div class="d-flex justify-content-center">
                    <button data-url="{{ route('taxonomies.create') }}" id="button-create" type="button" class="btn btn-info" >
        ...
        {{ Breadcrumbs::render('taxonomies.index') }}
    </x-page-header>
@stop
    ...
    <x-card>
        <div class="navbar navbar-light navbar-expand-lg py-lg-2 rounded-top shadow-0">
            ...
            <div class="navbar-collapse text-center text-lg-left flex-wrap collapse" id="inbox-toolbar-toggle-write">
                ...
                {{ $taxonomies->appends(request()->all())->links() }}
                    ...
                    @foreach($taxonomies as $taxonomy)
                        <tr>
                            <td class="table-inbox-checkbox">
                                <input type="checkbox" class="form-input-styled" data-fouc value="{{ $taxonomy->hash_key }}">
                            </td>
                            <td>
                                ...
                                <div class="list-icons">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            <a href="{{ route('taxonomies.edit', $taxonomy->hash_key) }}" class="dropdown-item"><i class="icon-pencil7"></i>{{ __('Edit') }}</a>
                                            <a href="javascript:void(0)" class="dropdown-item js-delete" data-url="{{ route('taxonomies.destroy', $taxonomy->hash_key) }}"><i class="icon-trash"></i> {{ __('Remove') }}</a>
                                        </div>
                                    </div>
                                </div>
                            </td>
